...
def uploadToS3(strFolder, strFile, awsCred):
    if awsCred["accessKeyId"] != "":
        print(
            "Uploading " + strFile + " to bucket " + awsCred["bucket"] + "/" + strFolder
        )
        ...
        s3.Bucket(awsCred["bucket"]).upload_file(
            file_handle.name, strFolder + "/" + strFile.split("/")[-1]
        )
    else:
...
def handler(event, context):
    ...
    c1.content = "<h1>Hello world</h1><p>Hello world.</p>"
    book.add_item(c1)
    book.toc = (
